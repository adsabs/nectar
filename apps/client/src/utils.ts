...
export const getFomattedNumericPubdate = (pubdate: string): string | null => {
  const { year, month } = parsePublicationDate(pubdate);
  return `${year}/${month}`;
};

export const parsePublicationDate = (pubdate: string): { year: string; month: string; day: string } => {
 ...
 */
const SKIP_KEYS = ['fq', 'fl', 'start', 'rows'];
export const parseQueryFromUrl = <TExtra extends Record<string, string | number | Array<string | number>>>(
  ...
  const normalizedParams = normalizeURLParams(params, SKIP_KEYS);
  const q = decodeURIComponent(normalizedParams?.q ?? '');
  const numPerPage = parseNumberAndClamp(
    normalizedParams?.n,
    APP_DEFAULTS.PER_PAGE_OPTIONS[0],
    APP_DEFAULTS.PER_PAGE_OPTIONS[APP_DEFAULTS.PER_PAGE_OPTIONS.length - 1],
  );
