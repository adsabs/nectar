...
const nextConfig = {
  ...
  async redirects() {
    return [// redirect bare abs links to /abstract by default
      {
        source: '/abs/:id', destination: '/abs/:id/abstract', permanent: true,
      },

      // redirect base export routes to bibtex by default
      {
        source: '/abs/:id/exportcitation', destination: '/abs/:id/exportcitation/bibtex', permanent: true,
      }, {
        source: '/search/exportcitation', destination: '/search/exportcitation/bibtex', permanent: true,
      },

      {
        source: '/user/settings', destination: '/user/settings/application', permanent: true,
      }, {
        has: [{
          type: 'host', key: 'page', value: 'localhost',
        }],
        source: '/help/:slug*',
  ...
  trailingSlash: false,
  publicRuntimeConfig: {
    apiHost: process.env.API_HOST_CLIENT, experiments: process.env.NEXT_PUBLIC_ENABLE_EXPERIMENTS?.split(',') || [],
  },
  serverRuntimeConfig: {
    apiHost: process.env.API_HOST_SERVER, baseCanonicalUrl: process.env.BASE_CANONICAL_URL,
  },
  images: {
    remotePatterns: [{
  ...
  compiler: {
    reactRemoveProperties: false, emotion: true,
...
const sentryConfig = {
  org: process.env.SENTRY_ORG, project: process.env.SENTRY_PROJECT, sentryOptions: {
    ...
    disableLogger: true,
  }, dsn: process.env.NEXT_PUBLIC_SENTRY_DSN, authToken: process.env.SENTRY_AUTH_TOKEN, silent: true,
};


export default composePlugins(withBundleAnalyzer({ enabled: !!process.env.ANALYZE }), (cfg) => () => withSentryConfig(cfg, sentryConfig))(nextConfig);
