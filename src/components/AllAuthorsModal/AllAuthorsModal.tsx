...
import { solrDefaultSortDirection, SolrSort } from '@/api/models';
...
export const AllAuthorsModal = ({ bibcode, label }: IAllAuthorsModalProps): ReactElement => {
  ...
  const preferredSort = normalizeSolrSort([`${preferredSortField} ${solrDefaultSortDirection[preferredSortField]}`]);
  const hasSortParam = router.asPath.includes('?') && new URLSearchParams(router.asPath.split('?')[1]).has('sort');
              ...
              <Box id="author-list-description">
                ...
                <Text
                  ...
                  dangerouslySetInnerHTML={{
                    __html: DOMPurify.sanitize(unwrapStringValue(data.docs[0].title)),
                  }}
                />
              </Box>
            )}
...
const createQuery = (type: 'author' | 'orcid', value: string, sort?: SolrSort[]): IADSApiSearchParams => {
  const defaultSort: SolrSort[] = type === 'author' ? ['score desc'] : ['date desc'];
  return { q: `${type}:"${value}"`, sort: sort ?? defaultSort };
};

const AuthorsTable = forwardRef<
  HTMLInputElement,
  { doc: IDocsEntity; onSearchClear: () => void; activeSort: SolrSort[] }
>(({ doc, onSearchClear, activeSort }, ref) => {
            ...
            <Tr key={`${author}${index}`}>
              ...
              <Td>
                {typeof author === 'string' && (
                  <SearchQueryLink
                    params={createQuery('author', author, activeSort)}
                    px={1}
